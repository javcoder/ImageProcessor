...
import com.javcoder.image.ImageConverter_tojpg;
import com.javcoder.image.ImageCutter;
...
public class ImageProcessor {
	...
	private class CutConvertListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			String texturePackerLocation = texturePackerLocationTextField.getText().trim();
			
			if(jpgQuality.equals("")) {
				jpgQuality = null;
			}
			
			String imageName = cutImage.substring(cutImage.lastIndexOf("\\") + 1, cutImage.lastIndexOf("."));
			String cutImageOutputDir = outputDir + "\\cut\\" + imageName;
			new File(cutImageOutputDir).mkdirs();
			
			try {
				ImageCutter.cut(cutImage, cutImageOutputDir, "bmp", Integer.valueOf(cutWidth), Integer.valueOf(cutHeight));
			} catch (NumberFormatException e1) {
				e1.printStackTrace();
				JOptionPane.showMessageDialog(null, "切割失败！", "信息提示",  JOptionPane.ERROR_MESSAGE);
			...
			try {
				ImageConverter_tojpg.process(texturePackerLocation, jpgQuality, cutImageOutputDir, outputDir);
			} catch (Exception e1) {
				e1.printStackTrace();
				JOptionPane.showMessageDialog(null, "转换失败", "信息提示",  JOptionPane.ERROR_MESSAGE);
			}
			
			JOptionPane.showMessageDialog(null, "切割、转换成功！", "信息提示",  JOptionPane.INFORMATION_MESSAGE);
		}
	}
