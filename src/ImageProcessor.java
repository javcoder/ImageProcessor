...
public class ImageProcessor {
	...
	private JTextField cutImageDirectoryChooserTextField;
	...
	private void initWidget() {
		...
		cutImageDirectoryChooserTextField = new JTextField();
		...
		cutImageDirectoryChooserTextField.setBounds(140, 10, 150, 20);
		...
		cutImageDirectoryChooserTextField.setEditable(false);
	...
	private void addWidget() {
		...
		frame.add(cutImageDirectoryChooserTextField);
	...
	private class FileChooserListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (source.equals(cutImageFileChooserButton)) {
				...
				if (path != null) {
					cutImageDirectoryChooserTextField.setText(path);
				}
	...
	private class CutConvertListener implements ActionListener {
		@Override
		public void actionPerformed(ActionEvent e) {
			String cutImageDir = cutImageDirectoryChooserTextField.getText()
					.trim();
			...
			// validate
			if (empty(cutImageDir) || empty(outputDir) || empty(cutWidth)
					|| empty(cutHeight) || empty(texturePackerLocation)) {
			...
			String cutImageOutputDir = outputDir + File.separator + ".cut";
			if(!new File(cutImageOutputDir).mkdirs()) {
				System.err.println("mkdir error! dir: " + cutImageOutputDir);
			}

			try {
				ImageCutter.process(cutImageDir, cutImageOutputDir,
						"bmp", Integer.valueOf(cutWidth),
						Integer.valueOf(cutHeight));
